...
Agent = D3QNAgent(16, len(get_state(env)))
...
if IS_TRAIN:
    for i_episode in range(n_episode):
        ...
        for i_step in range(n_step_per_episode):  # range内是0.1/0.001 = 100  i_step从0开始
            ...
            """根据环境得到action"""
            action = Agent.choose_action(state, epsi)
            ...
            Agent.store_transition(state, state_new, action,train_reward)  # add entry to this agent's memory

            # training this agent
            if time_step % mini_batch_step == mini_batch_step - 1:
                loss_val_batch = Agent.learn().item()
