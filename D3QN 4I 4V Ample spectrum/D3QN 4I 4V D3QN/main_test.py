...
with g.as_default():
    # ============== Training network ========================
    x = tf.placeholder(tf.float32, [None, n_input])  # [None, n_input]表示列为n_input的长度，行不定
    ...
    ###---------------------------------
    w_5 = tf.Variable(tf.truncated_normal([n_hidden_3, n_state_output], stddev=0.1))
    ###----------------------------------
    ...
    ###---------------------------------
    b_5 = tf.Variable(tf.truncated_normal([n_state_output], stddev=0.1))
    ###---------------------------------
    ...
    ## y的shape是（？，16），每一列应该是选择此动作的一个概率值，概率值在0-1之间
    y1 = tf.nn.relu(tf.add(tf.matmul(layer_3, w_4), b_4))  # action value
    y2 = y1 - tf.reduce_mean(y1, axis=1, keep_dims=True)
    # tf.argmax(input,axis)根据axis取值的不同返回每行或者每列最大值的索引，axis=1的时候，将每一行最大元素所在的索引记录下来，最后返回每一行最大元素所在的索引数组。
    # 比如 test = array([1, 2, 3])
    # 3最大，返回的就是索引2

    ###---------------------------------
    y_state_value = tf.nn.relu(tf.add(tf.matmul(layer_3, w_5), b_5))  # (?,1)
    ###---------------------------------
    y = y2 + y_state_value
    ...
    action_one_hot = tf.one_hot(g_action, n_output, 1.0, 0.0, name='action_one_hot')  # n_output = 16

    # 调用reduce_sum(arg1, arg2)时，参数arg1即为要求和的数据，arg2有两个取值分别为0和1，当arg2 = 1时，是横向对矩阵求和
    q_acted = tf.reduce_sum(y * action_one_hot, reduction_indices=1,
                            name='q_acted')  # reduction_indices参数的值为1的时候，是第1维对应位置相加。

    # 损失函数计算（预测值与误差值的差距）；误差反向传播修改权重矩阵值
    ...
    ###---------------------------------
    w_5_p = tf.Variable(tf.truncated_normal([n_hidden_3, n_state_output], stddev=0.1))
    ###----------------------------------
    ...
    ###---------------------------------
    b_5_p = tf.Variable(tf.truncated_normal([n_state_output], stddev=0.1))
    ###---------------------------------
    ...
    # y_p的shape是（？，16），每一列应该是选择此动作的一个概率值，概率值在0-1之间
    y_p1 = tf.nn.relu(tf.add(tf.matmul(layer_3_p_b, w_4_p), b_4_p))
    y_p2 = y_p1 - tf.reduce_mean(y_p1, axis=1, keep_dims=True)
    ###---------------------------------
    y_p_state_value = tf.nn.relu(tf.add(tf.matmul(layer_3_p_b, w_5_p), b_5_p))
    ###---------------------------------
    y_p = y_p2 + y_p_state_value
    g_target_q_idx = tf.placeholder('int32', [None, None], 'output_idx')

    # tf.gather_nd是按照g_target_q_idx的格式从y_p里抽取切片，并合并为一个tensor
...
def q_learning_mini_batch(current_agent, current_sess):
    ...
    if current_agent.double_q:  # double q-learning
        ...
        q_t_plus_1 = current_sess.run(target_q_with_idx, {x_p: batch_s_t_plus_1,
...
def update_target_q_network(sess):
    ...
    sess.run(w_5_p.assign(sess.run(w_5)))
    ...
    sess.run(b_5_p.assign(sess.run(b_5)))
...
if IS_TEST:
    ...
    for idx_episode in range(n_episode_test):
        ...
        env.individual_time_limit = (env.time_slow / 2) * np.ones((env.n_Veh, env.n_neighbor))
        ...
        print('marl', round(np.average(V2I_rate_per_episode), 2))
